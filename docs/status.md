...
| **A late progress update can leave the scan looking like it is still running** (found 2026-09-24, not yet reproduced). Each progress callback in `start_scan` writes `scan_status` and `scan_progress` from a newly spawned task, while the final Completed or Error status is written directly once `process_projects_with_progress` returns. Nothing orders the two, so a task scheduled late can overwrite the final status with an earlier phase. `begin_scan` then counts the scan as running and refuses every project scan, plugin scan and sample check until the process restarts. The window is small. `start_plugin_scan` and `start_sample_check` write their status directly and don't have this problem. | `src/services/system.rs` (`start_scan`) | Medium: unlikely, but it locks out every scan until a restart |
...
| **The watcher event stream holds a runtime worker** (found 2026-09-24). `start_watcher_event_stream` loops on a blocking `std::sync::mpsc::Receiver::recv()` inside `tokio::spawn`, so one tokio worker is blocked for as long as a client streams watcher events. This is the same kind of problem as the `start_scan` entry above. | `src/services/system.rs` (`start_watcher_event_stream`) | Low |
| **Adding projects parses them on an async worker** (found 2026-09-24). `add_single_project` and `add_multiple_projects` call `Project::new`, which does a synchronous gunzip and full parse, directly inside an async fn. That's cheap for one file and blocks a runtime worker for a large multi-add. | `src/services/system.rs` | Low |
